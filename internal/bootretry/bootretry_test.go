...
func TestRetry_SucceedsFirstTimeWithoutWaiting(t *testing.T) {
	...
	// service starts serving, so every second here is a second of boot
	// latency.
...
// The retry must NOT weaken any fail-closed rule: a genuinely unreachable
...
func TestRetry_GivesUpAndReportsTheLastError(t *testing.T) {
	...
	if err == nil {
		t.Fatal("retry must fail when every attempt fails; a silent success would hide a real outage")
	}
