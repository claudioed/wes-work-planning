...
// the OTel pgx tracer installed (mirroring the OLTP postgres.Connect). It is
// used by the writer (cmd/wes-projector).
...
// (default_transaction_read_only=on). The reader process (cmd/wes-reports)
// uses this so a bug there cannot mutate the read model even if the database
// role itself is not read-only — defence in depth on top of the read-only
// ANALYTICS_DATABASE_URL role (ADR-0011).
