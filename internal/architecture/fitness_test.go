// Package architecture holds fitness tests (the Go analogue of ArchUnit, via
// github.com/arch-go/arch-go) that enforce the hexagonal/ports-and-adapters
...
// consumer groups outright (the analytics projector's AnalyticsConsumerGroup
// constant is a deliberate, correct exception — see its own doc comment:
// only one instance of that consumer ever runs, so there's nothing to
// collide with) — it bans the shape that caused the incident: a GroupID
// assigned directly from an inline string literal rather than through a
// named symbol (const, var, or function call) that a reviewer can trace
// back to its definition and reasoning.
